...
class Window(CTkFrame):
    def __init__(self, parent):
        ...
        label = CTkLabel(self, text="ALMAVIVA SCRIPT",
                         font=CTkFont(family="Segoe UI", size=15))
        label.place(relx=0.5, rely=0.1, anchor=CENTER)

        self.email_lbl = CTkLabel(self, text=render_text(
            "اسم المستخدم"), font=CTkFont(family="Segoe UI", size=15))
        ...
        self.password_lbl = CTkLabel(self, text=render_text(
            "كلمة المرور"), font=CTkFont(family="Segoe UI", size=15))
        ...
        self.label3 = CTkLabel(
            self, text="", font=CTkFont(size=20))
        self.label2 = CTkLabel(
            self, text="..", font=CTkFont(family="Segoe UI", size=15))

        self.time_lbl = CTkLabel(
            self, text=render_text("وقت التنفيذ"),
            font=CTkFont(family="Segoe UI", size=15)
        )

        self.time_lbl.place(relx=0.85, rely=0.46, anchor=E)

        self.hours_lbl = CTkLabel(self, text=render_text(
            "الساعة"), font=CTkFont(family="Segoe UI", size=13))
        ...
        self.minutes_lbl = CTkLabel(self, text=render_text(
            "الدقيقة"), font=CTkFont(family="Segoe UI", size=13))
        ...
        self.select_destination = CTkEntry(
            self, placeholder_text="italy or spain")
        self.select_destination.place(relx=0.5, rely=0.72, anchor=CENTER)

        self.select_trip_date_lbl = CTkLabel(
            self, text=render_text("يوم الرحلة"),
            font=CTkFont(family="Segoe UI", size=15))
        ...
        self.state_lbl = CTkLabel(self, text=render_text(
            "انتظار..."), font=CTkFont(family="Segoe UI", size=13), text_color=info)
        self.state_lbl.place(relx=0.2, rely=0.6, anchor=CENTER)

        self.start_program = CTkButton(
            self, text=render_text("بدأ البرنامج"), font=CTkFont(family="Segoe UI", size=15), command=lambda: self.update_timer(
                self.get_time()[0], self.get_time()[1]))
        self.start_program.place(relx=0.6, rely=0.82, anchor=CENTER)

        self.stop_prgram = CTkButton(
            self, text=render_text("ايقاف البرنامج"), font=CTkFont(family="Segoe UI", size=15), command=lambda: self.update_timer(
                self.get_time()[0], self.get_time()[1], 0),
            fg_color="#FF204E", hover_color="#A0153E",
            state=DISABLED)
        self.stop_prgram.place(relx=0.4, rely=0.82, anchor=CENTER)

        self.components = [self.hours_entry,
                           self.email_entry,
                           self.minutes_entry,
                           self.password_entry,
                           self.time_type,
        ...
        try:
            if condition:
                ...
                next_date = current_date.replace(
                    hour=det_h, minute=det_min, second=5) - (current_date + timedelta(days=1))
                hours, remainder = divmod(next_date.seconds, 3600)
                minutes, seconds = divmod(remainder, 60)
                missing_time = "{0}:{1}:{2}".format((hours if hours >= 10 else '0' + str(
                    hours)), (minutes if minutes >= 10 else '0' + str(minutes)), (seconds if seconds >= 10 else '0' + str(seconds)))
                self.label2.configure(text=missing_time)
                self.label3.configure(text=render_text(
                    "الوقت المتبقي حتي يتم تنفيذ البرنامج"), text_color="white")

                if hours == 0 and minutes == 0 and seconds == 0:
                    self.driver_thread = Thread(target=start_program, args=(
                    ...
                    self.label3.configure(
                        text=render_text("البرنامج بدأ التنفيذ"), font=("Muli", 17, 'bold'), text_color=success)
                    self.state_lbl.configure(text=render_text("بدأت عملية التنفيذ بنجاح.."), text_color=success)

                    self.restoring_function = self.after(
                        100, self.driver_thread.start)
                ...
                self.label3.configure(text=render_text(
                    "البرنامج توقف"), text_color="#FF204E")
                self.enable_components()
    ...
    def get_time(self):
        ...
        if hours > 12 or hours == 0:
            messagebox.showerror(
                title="خطأ", message=" برجاء ادخال عدد الساعات بطريقة صحيحة حيث عدد الساعات ينحصر بين ال 1 الي 12")
            return
        if mins >= 60:
            messagebox.showerror(
                title="خطأ", message=" برجاء ادخال عدد الساعات بطريقة صحيحة حيث عدد الدقائق ينحصر بين ال 0 الي 59")
            return
        return (int(hours) % 12 if self.time_type.get() != "م" else (int(hours) + 12) % 24, int(mins))
    ...
    def get_days_in_month(self):
        ...
        return list(map(str, range(current_day.day, (next_month - current_day).days + 1)))
    ...
    def validation(self):
        if self.email_entry.get() == "" or self.password_entry.get() == "" or self.select_destination.get() == "":
            messagebox.showerror(
                title="خطأ", message="برجاء تعبئة جميع الحقول")
            return False
        return True
