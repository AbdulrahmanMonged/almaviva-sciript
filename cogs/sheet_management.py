...
class dbManagement:
    ...
    async def start_excution(self, logged_user, logged_password, key=None):
        try:
    ...
    async def finish_excution(self, paymentGate, key=None):
        try:
            async with await psycopg.AsyncConnection.connect(URI) as db:
                async with db.cursor() as curr:
                    if not key:
    ...
    async def set_video_call(self, videoCallUrl, key=None):
        try:
            async with await psycopg.AsyncConnection.connect(URI) as db:
                async with db.cursor() as curr:
                    if not key:
                        key = self.key
                    await curr.execute(
                        "UPDATE operations SET videocall_url = %s WHERE id = %s",
                        ( videoCallUrl, key),
                    )
                    await db.commit()
            ...
            print(e)
            await self.finish_excution(videoCallUrl)

    async def write_user(self, siteName, sitePassword, key=None):
        try:
            async with await psycopg.AsyncConnection.connect(URI) as db:
                async with db.cursor() as curr:
                    if key is None:
                        if self.key is None:
                            self.key = uuid4()
                        key = self.key
                    await self.start_excution(secretvars.username, secretvars.password, key=key)
                    await curr.execute(
                        "UPDATE operations SET site_username = %s, site_password = %s WHERE id = %s",
                        (siteName, sitePassword, key),
                    )
                    await db.commit()
        except Exception as e:
            print(e)
    ...
    async def check_passport(self, key=None):
        try:
            async with await psycopg.AsyncConnection.connect(URI) as db:
                async with db.cursor() as curr:
                    if key is None:
