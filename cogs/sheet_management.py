...
headers = {"Authorization": f"Bearer {secretvars.AUTH_CODE}", "Content-Type": "application/json"}
...
def start_excution(site_name, site_password):
    ENDPOINT_ID = (
        f"https://api.sheety.co/2c40846f41f1ee967444320646c06b9e/almavivaLogs/main/{secretvars.ID}"
    )
    current_date = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
...
def update_login_status(status):
    ...
    body = {"main": {"siteLoginDate": current_date, "loginStatus": status, "statusOfTheOperation": "Stuck on appointment Page"}}
    requests.put(ENDPOINT_ID, json=body)
